...
# The 403 PERMISSION_DENIED (API_KEY_SERVICE_BLOCKED) error indicates that requests 
# to the public Gemini API (AI Studio) are blocked in this environment.
# Switching to the Vertex AI backend is the recommended solution.
PROJECT_ID = os.getenv("GOOGLE_CLOUD_PROJECT", "my-sandbox-project1-499520")
...
def main():
    print("Connecting to local persistent ChromaDB context...")
    ...
    print(f"Beginning embedding process for {len(HISTORICAL_EVENTS_CORPUS)} events utilizing text-embedding-004...")
    ...
    doc_texts = [
        f"Title: {e['title']} | Date: {e['date']} | Description: {e['description']} | Outcome: {e['outcome']}"
        for e in HISTORICAL_EVENTS_CORPUS
    ]

    print(f"Beginning batch embedding process for {len(doc_texts)} events utilizing text-embedding-004...")
    try:
        # Batch embedding is significantly faster and more reliable than looping for small sets
        ...
        for i, event in enumerate(HISTORICAL_EVENTS_CORPUS):
            ids.append(event["id"])
            embeddings.append(response.embeddings[i].values)
            metadatas.append({
                "title": event["title"],
                "date": event["date"],
                "sector": event["sector"],
            ...
            documents.append(doc_texts[i])

    except Exception as e:
        print(f"Failed embedding of corpus: {e}")
        sys.exit(1)
            
    print("Writing structural records to local ChromaDB collection...")
    collection.add(
        ids=ids,
    ...
    print("Seeding completed successfully! ChromaDB populated with 50 curated high-impact events.")
